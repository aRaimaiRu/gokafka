...
func NewAccountRepository(db *gorm.DB) AccountRepository {
	db.Table("first_banks").AutoMigrate(&BankAccount{})
	return accountRepository{db}
}

func (obj accountRepository) Save(bankAccount BankAccount) error {
	return obj.db.Table("first_banks").Save(bankAccount).Error
}

func (obj accountRepository) Delete(id string) error {
	return obj.db.Table("first_banks").Where("id=?", id).Delete(&BankAccount{}).Error
}

func (obj accountRepository) FindAll() (bankAccounts []BankAccount, err error) {
	err = obj.db.Table("first_banks").Find(&bankAccounts).Error
	return bankAccounts, err
}

func (obj accountRepository) FindByID(id string) (bankAccount BankAccount, err error) {
	err = obj.db.Table("first_banks").Where("id=?", id).First(&bankAccount).Error
